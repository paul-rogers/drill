...
public class RegexRecordReader extends AbstractRecordReader {
  ...
  private void setupPattern() {
    ...
    if (groupCount == 0) {
      throw UserException
        .validationError()
        .message("Regex contains no groups: \"%s\"", formatConfig.regex)
        .build(logger);
    }
  }
  ...
  private void setupColumns() {
    ...
    List<String> fields = Splitter.on(Pattern.compile("\\s*,\\s*")).splitToList(fieldStr);
    if (groupCount > fields.size()) {
      logger.warn(
          ...
          fields.toString(), formatConfig.regex);
    }
