...
public class KafkaPartitionScanSpecBuilder extends
  ...
  public List<KafkaPartitionScanSpec> visitBooleanOperator(BooleanOperator op, Void value)
    ...
    if (op.getName().equals(BooleanOperator.OR_FN)) {
      ...
      for (LogicalExpression expr : args) {
        ...
        if (parsedSpec != null) {
          ...
          // At any level, all arguments of booleanOr should support pushdown, else return null
          return null;
        }
      }
    } else { // booleanAnd
      ...
      for (LogicalExpression expr : args) {
        ...
        // parsedSpec is null if expression cannot be pushed down
        if (parsedSpec != null) {
          // Store topic-partitions returned from new spec.
          Set<TopicPartition> partitionsInNewSpec = Sets.newHashSet();
          ...
          // For BooleanOperator.AND_FN, handle the case where condition is on `kafkaPartitionId`.
  ...
  private List<KafkaPartitionScanSpec> createScanSpecForTimestamp(String functionName,
    ...
    for (TopicPartition partitions : topicPartitions) {
      timesValMap.put(partitions, functionName.equals(FunctionCall.GT_FN) ? fieldValue+1 : fieldValue);
    }
