...
/**
 * Priority queue copier that relies on a generated implementation
 * for compare and copy operations. Allows multiple generated
 * implementations. Avoids copying the driver code when using the
 * byte-code fixup method of code generation.
 */

public class PriorityQueueCopierDriver {
  ...
  private final PriorityQueueCopier copier;

  public PriorityQueueCopierDriver(PriorityQueueCopier copier) {
    this.copier = copier;
  }
  ...
  public void setup(BufferAllocator allocator, VectorAccessible hyperBatch, List<BatchGroup> batchGroups,
    ...
    copier.setup(allocator, hyperBatch, batchGroups, outgoing);
    copier.doSetup(hyperBatch, outgoing);
  ...
  public int next(int targetRecordCount) {
    ...
    for (int outgoingIndex = 0; outgoingIndex < targetRecordCount; outgoingIndex++) {
      ...
      try {
        copier.doCopy(compoundIndex, outgoingIndex);
