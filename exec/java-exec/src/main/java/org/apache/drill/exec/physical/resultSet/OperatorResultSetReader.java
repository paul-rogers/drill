 ...
 * </ol>
 */
public interface OperatorResultSetReader extends ResultSetReader {

  int schemaVersion();

  /**
   * Detach the batch of data from this reader. Does not
   * release the memory for that batch. Prevents {@code next()}
   * or {@code close()} from releasing memory.
   */
  void detach();
