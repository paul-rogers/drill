...
public class RequestedTupleImpl implements RequestedTuple {
  ...
  public boolean isConsistentWith(ColumnMetadata col) {
    RequestedColumnImpl colProj = getImpl(col.name());
    return colProj == null ? true : colProj.isConsistentWith(col);
  }

  @Override
  public boolean isConsistentWith(String name, MajorType type) {
    RequestedColumnImpl colProj = getImpl(name);
    return colProj == null ? true : colProj.isConsistentWith(type);
  }

  @Override
  public RequestedTuple mapProjection(String colName) {
    RequestedColumnImpl colProj = getImpl(colName);
    return colProj == null ? ImpliedTupleRequest.NO_MEMBERS : colProj.mapProjection();
  }
