...
import org.apache.drill.exec.vector.accessor.reader.VectorAccessors.NullVectorAccesor;
...
public class JoinReaderBuilder extends BaseReaderBuilder {
  ...
  private void buildTable(
    ...
    if (implicitTable.child == null) {
      readers = buildMapMembers(mapVector, mdProvider);
    } else {
      readers = buildMap(mapVector, mdProvider, implicitTable.child);
    }
    AbstractObjectReader mapReader = MapReader.build(
        rootColumn, new SingleVectorAccessor(vector), readers);
    if (type.getMode() == DataMode.REPEATED) {
      implicitTable.reader = ArrayReaderImpl.buildTuple(
          null, new SingleVectorAccessor(vector), mapReader);
    } else {
  ...
  private List<AbstractObjectReader> buildMap(AbstractMapVector mapVector,
      MetadataProvider provider, ImplicitJoinReaderImpl.ImplicitTableImpl targetTable) {
    List<AbstractObjectReader> readers = new ArrayList<>();
    int i = 0;
