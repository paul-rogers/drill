 ...
 * <p>
 * You can use this class directly for the simplest of cases: a single
 * minor fragment that requires only schema, table and column names.
 * This lets you get something working quickly. You can then extend
 ...
 * "scan specification"
 * as the "scanSpec" attribute in JSON. Subclasses can add fields to
 * this class (if used only by the group scan) or to the scan spec
 * (if shared with the sub scan.)</li>
 ...
 * config, and gets that from the storage plugin itself.</li>
 ...
 * copy. This base class automatically invokes the the copy constructor
 * {@link #BaseGroupScan(BaseGroupScan)}. The code will invoke
 * that constructor on your derived class using Java introspection.
 * <p>
 ...
 * {@link BaseStoragePlugin#newGroupScan(String, BaseScanSpec,
 * SessionOptionManager, MetadataProviderManager)}
 * or the simpler {@link BaseStoragePlugin#newGroupScan(String, BaseScanSpec)} to
 * create a group scan. You must override one of these methods if you create
 * your own subclass.
 *
 ...
 *
 * Drill has the idea of a scan specification, as mentioned above. The
 * "Base" classes run with the idea to define a {@link BaseScanSpec} that
 * holds data shared between the group and sub scans. This approach
 * avoids the need to copy and serialize the data in two places, and
 * allows the automatic copies described above.
 * <p>
 * Your subclass should include other query-specific data needed at
 * both plan and run time such as file locations, partition information,
 * filter push-downs and so on.
 * <p>
 * Some existing plugins copy the data from group to sub scan, but
 * this is not necessary if you use the scan spec class.
 *
 * <h4>Costs</h4>
 *
...
public abstract class BaseGroupScan extends AbstractGroupScan {
  ...
  public BaseGroupScan(
    ...
    super(userName);
    this.storagePlugin = BaseUtils.resolvePlugin(engineRegistry, config);
    this.columns = columns;
  }
