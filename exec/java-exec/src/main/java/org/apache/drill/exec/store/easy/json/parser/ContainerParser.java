...
public abstract class ContainerParser extends AbstractParser {
  ...
  protected final TupleProjection projection;

  public ContainerParser(JsonLoaderImpl loader, String key, TupleProjection projection) {
    super(loader, key);
    this.projection = projection;
  }

  public ContainerParser(JsonElementParser parent, String key, TupleProjection projection) {
    super(parent, key);
    this.projection = projection;
  }
  ...
  protected ArrayParser objectArrayParser(String key) {
    ...
    return new ArrayParser.ObjectArrayParser(this, key, arrayWriter,
        new ObjectParser(this, key, arrayWriter.tuple(),
            projection.map(key)), projection.map(key));
  }
