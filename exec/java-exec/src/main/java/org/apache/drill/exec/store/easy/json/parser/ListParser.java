...
public class ListParser extends ArrayParser {
   ...
   * on that basis.
   */
  protected static class NullElementParser extends AbstractParser
      implements NullTypeMarker {
    ...
    public boolean parse() {
      ...
      JsonElementParser newParser = listParser.detectElementParser(token);
  ...
  public ListParser(ContainerParser parent, String key, ArrayWriter writer) {
    // No projection filtering within lists.
    super(parent, key, writer, TupleProjectionImpl.projectAll());
  ...
  protected ObjectParser objectElementParser(String key) {
    ...
    // Ambiguity. We are in a LIST and we want to project an
    // object. We may have ARRAY<ARRAY<MAP>>. For now, we don't
    // try to track projection this deep into the data structure,
    // so just project everything.
    return new ObjectParser(this, key, tupleWriter,
        TupleProjectionImpl.projectAll());
  }
  ...
  private JsonElementParser inferElementFromHint() {
    ...
    switch (type.getMinorType()) {
      case MAP:
        // No projection filtering in a MAP within a list; project
        // all members.
