...
public class TestSimpleLimit extends ExecTest {
  ...
  @Test
  public void testLimit(@Injectable UserServer.UserClientConnection connection) throws Throwable {
    verifyLimitCount(connection, "test1.json", 5);
  }

  @Test
  public void testLimitNoEnd(@Injectable UserServer.UserClientConnection connection) throws Throwable {
    verifyLimitCount(connection, "test3.json", 95);
  }
  ...
  public void testLimitAcrossBatches(@Injectable UserServer.UserClientConnection connection) throws Throwable {

    verifyLimitCount(connection, "test2.json", 69999);
    final long start = 30000;
    final long end = 100000;
    final long expectedSum = (end - start) * (end + start - 1) / 2; //Formula for sum of series

    verifySum(connection, "test4.json", 70000, expectedSum);
  }

  private void verifyLimitCount(UserServer.UserClientConnection connection, String testPlan, int expectedCount) throws Throwable {
  ...
  private void verifySum(UserServer.UserClientConnection connection, String testPlan, int expectedCount, long expectedSum) throws Throwable {
    final PhysicalPlanReader reader = PhysicalPlanReaderTestFactory.defaultPhysicalPlanReader(c);
    final PhysicalPlan plan = reader.readPhysicalPlan(Files.toString(FileUtils.getResourceAsFile("/limit/" + testPlan), Charsets.UTF_8));
    final FunctionImplementationRegistry registry = new FunctionImplementationRegistry(c);
    final FragmentContext context = new FragmentContext(bitContext, PlanFragment.getDefaultInstance(), connection, registry);
