...
import org.apache.drill.exec.physical.impl.scan.v3.schema.MutableTupleMetadata;
...
public class TestImplicitColumnResolver extends SubOperatorTest {
  ...
  public void testFileImplicitColumnSelection() {
    ...
    List<ColumnHandle> cols = parseFixture.tracker.schema().columns();
  ...
  public void testRevisedWildcard() {
    ...
    assertTrue(result.columns().isEmpty());
    assertTrue(result.schema().isEmpty());
    assertTrue(parseFixture.tracker.schema().columns().isEmpty());
  }
  ...
  public void testImplicitWithDefinedSchema() {
    ...
    MutableTupleMetadata internalSchema = schemaTracker.schema();
