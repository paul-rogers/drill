...
public class DummyBatchReader implements ManagedReader<SchemaNegotiator> {
  ...
  private static final Logger logger = LoggerFactory.getLogger(DummyBatchReader.class);
  ...
  public boolean open(SchemaNegotiator negotiator) {
    ...
    if (negotiator.rootProjection().type() != TupleProjectionType.SOME) {
      ...
      for (SchemaPath column : columns) {
        if (! column.isLeaf()) {
          throw UserException.validationError()
            .message("Dummy reader only supports simple columns")
            .addContext("Column", column.toExpr())
            .addContext(negotiator.parentErrorContext())
            .build(logger);
        }
        builder.add(column.rootName(), MinorType.VARCHAR);
      }
